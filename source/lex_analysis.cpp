...
size_t add_id(Lexer* lexer, IDType type) 
{
    // Проверка на валидность указателей
    if (!lexer || !lexer->tokens || !lexer->scopes) {
        return ERR_VAL_SIZE_T;
    }

    // Проверка выхода за границы
    //if (lexer->word_cnt >= lexer->words_amt || 
    //    lexer->cur_scope >= MAX_SCOPES) {
    //    return ERR_VAL_SIZE_T;
    //}
...
bool process_declaration_token(Lexer* lexer)
{
    ...
    {
        set_token(FUNC_SET, type, lexer);
        id_type = FUNC_ID;
        token_type = FUNC;
    }
    else
        set_token (VAR_SET, type, lexer);
    ...
    set_token(token_type, id_num, lexer);
...
{   
    ...
    set_token(KEY_WORD, keyword_type, lexer);
...
{
    //printf("id\n");
    size_t id_num = check_id(lexer);

    if (id_num == ERR_VAL_SIZE_T) return false;
